...
export default function AdminUsers() {
  ...
  const initialTab = (searchParams.get("tab") as "users" | "approvals") || "users";
  ...
  const fetchApplications = async () => {
    ...
    try {
      ...
      toast({ title: "Lỗi", description: "Không tải được danh sách hồ sơ.", variant: "destructive" });
    } finally {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 mt-1">Quản lý học sinh, quản trị viên, và duyệt hồ sơ giáo viên</p>
          </div>
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                ...
                <DialogDescription>Tạo tài khoản mới cho học sinh hoặc quản trị viên</DialogDescription>
              </DialogHeader>
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
          ...
          <TabsContent value="approvals" className="space-y-6">
            <div className="flex items-center gap-4 p-4 bg-white rounded-lg border border-gray-200">
              ...
              <Button variant="outline" onClick={fetchApplications} disabled={loadingApps}>
                {loadingApps ? (
                  <>
                ...
                <TableBody>
                  {filteredApps.map((i) => (
                    <TableRow key={i.id} className="hover:bg-gray-50">
                      ...
                      <TableCell className="text-gray-600">{new Date(i.createdAt).toLocaleDateString("vi-VN")}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {i.status === "pending" ? (
                          <div className="flex gap-2 justify-end">
                            ...
                            <Button size="sm" variant="destructive" onClick={() => setRejectDialog({ open: true, id: i.id, reason: "" })}>
              ...
              <DialogContent className="sm:max-w-[480px]">
                ...
                <div className="grid gap-4 py-4">
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Input id="reason" value={rejectDialog.reason} onChange={(e) => setRejectDialog((s) => ({ ...s, reason: e.target.value }))} className="col-span-3" />
                  </div>
                </div>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setRejectDialog({ open: false, id: null, reason: "" })}>Hủy</Button>
                  <Button variant="destructive" onClick={reject} disabled={!rejectDialog.reason.trim()}>Từ chối</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
          </TabsContent>
