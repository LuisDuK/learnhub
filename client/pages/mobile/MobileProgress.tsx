...
const month = Array.from({ length: 30 }, (_, i) => ({
  day: String(i + 1),
  val: Math.floor(Math.random() * 70) + 10,
}));
  ...
  return (
    <div className="space-y-4 p-4">
      <div className="rounded-2xl border border-primary/20 bg-gradient-to-r from-primary/10 via-accent/10 to-secondary/10 p-3">
        <div className="grid grid-cols-3 gap-3">
          {stats.map((s) => (
            <div
              key={s.label}
              className="rounded-xl bg-white/60 p-3 text-center shadow-sm backdrop-blur dark:bg-card/60"
            >
      ...
      <Card className="border-accent/30">
        <CardHeader className="flex items-center justify-between p-4 pb-0">
          <CardTitle className="text-base">
            Hoạt động {range === "week" ? "tuần này" : "tháng này"}
          </CardTitle>
          <div className="inline-flex rounded-xl border border-primary/20 bg-background p-1 text-xs">
            <button
              onClick={() => setRange("week")}
              className={`rounded-lg px-2 py-1 ${range === "week" ? "bg-primary text-white" : "text-foreground"}`}
            >
              Tuần
            </button>
            <button
              onClick={() => setRange("month")}
              className={`rounded-lg px-2 py-1 ${range === "month" ? "bg-primary text-white" : "text-foreground"}`}
            >
              Tháng
            </button>
          </div>
        ...
        <CardContent className="p-4">
          <div className="flex items-end justify-between gap-1">
            {data.map((b) => (
              ...
              >
                <div
                  className="w-6 rounded-full bg-gradient-to-t from-primary to-accent"
                  style={{ height: `${Math.max(10, b.val)}px` }}
                />
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link
          to="/m/plan"
          className="rounded-2xl border border-primary/20 bg-gradient-to-br from-primary/10 to-accent/10 p-4 text-center"
        >
        ...
        <Link
          to="/m/plan/create"
          className="rounded-2xl border border-accent/30 bg-gradient-to-br from-accent/10 to-secondary/10 p-4 text-center"
        >
