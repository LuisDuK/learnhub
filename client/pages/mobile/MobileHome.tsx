...
const quick = [
  ...
  },
  {
    ...
    desc: "Từng bước",
    icon: Flag,
    color: "bg-gradient-to-br from-accent/10 to-secondary/10",
  },
  {
    to: "/m/progress",
    title: "Thử thách",
  ...
  },
  {
    ...
    desc: "PDF, bài tập",
    icon: FileText,
    color: "bg-gradient-to-br from-primary/10 to-secondary/10",
  },
];
      ...
      <div className="grid grid-cols-2 gap-3">
        {quick.map((q) => (
          <Link
            key={q.title}
            to={q.to}
            className={`rounded-2xl border border-primary/20 p-4 ${q.color} hover:scale-[1.02] transition-transform`}
          >
            <div className="flex items-center justify-between">
              <div>
          ...
          <div className="no-scrollbar -mx-1 flex snap-x gap-3 overflow-x-auto px-1">
            {enrolled.map((c) => (
              <Link
                key={c.id}
                to={`/m/course/${c.id}`}
                className={`snap-start w-64 shrink-0 rounded-2xl border border-primary/20 p-4 shadow-md bg-gradient-to-br ${c.color}`}
              >
                <p className="font-semibold">{c.title}</p>
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Hoàn thành {c.progress}%
                </p>
